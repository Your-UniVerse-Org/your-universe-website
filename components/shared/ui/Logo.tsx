...
/* ── Official orbital icon path (extracted from /public/DARK BLUE LOGO.svg,
       viewBox 0 0 2651 1615, icon bounding box ≈ 992,411 → 1658,1204)  ── */
const MARK_PATH =
...
export default function Logo({
  ...
  if (variant === "wordmark") {
    const markH = height;
    const markW = Math.round(markH * (666 / 793));
    return (
      <span
        className={className}
        style={{
 ...
 */
export function LogoMark({ size = 36, white = true }: { size?: number; white?: boolean }) {
  const fill = white ? "#FFFFFF" : "#0F172A";
  const w = Math.round(size * (666 / 793));
  return (
    <svg
      viewBox="992 411 666 793"
      width={w}
